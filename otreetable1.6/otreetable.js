...
function oTreeTable(id, tb, _cfg){
	...
	_.config = {
		...
		//是否缓存被折叠的节点
		isCache: _._checkBoolean([_cfg.isCache, _cfg.isCookie], true),
		//缓存Cookie过期时长，单位：分钟
		expireMinutes: _cfg.expireMinutes || 0
	};
	if(typeof _.config.isCache != 'boolean'){
		_.config.isCache = true;
	}
...
oTreeTable.prototype = {
	_checkBoolean: function(arr,val){
		for(var i in arr){
	...
	_buildPid: function(pid){
		return 'pid_' + pid;
	},
	_buildId: function(id){
		return 'id_' + id;
	},
	_buildRowId: function(id){
		return this.id + 'tr_' + id;
	},
	_buildSwitchId: function(id){
		return this.id + 'tr_a_' + id;
	},
	_buildIconId: function(id){
		return this.id + 'tr_i_' + id;
	},
	_getRowObj: function(id){
		return document.getElementById(this._buildRowId(id));
	},
	_getSwitchObj: function(id){
		return document.getElementById(this._buildSwitchId(id));
	},
	_checkHasChild: function(pid){
...
oTreeTable.prototype._buildSwitch = function(rData, hasChild){
	var arr = [
		(this.config.clickIcon ? '<a onclick="' + this.id + '.toggle(' + rData.id + ');' + this.id + '.cacheExpand();">' : ''),
...
oTreeTable.prototype.build = function(pid, obj, arr){
	...
	for(var i in arr){
		...
		if(d.pid == pid){
			...
			row.lang = d.lang;
			row.data = {id:d.id, pid:d.pid, level:d.level};
			if(d.click != null){
				row.onclick = d.click;
			}
			if(d.dblclick != null){
...
oTreeTable.prototype.expand = function(id, aid){
	var _ = this;
	console.log(_._getRowData(id));
	if(_._getRowData(id) == undefined){
		return false;
	}
	if(aid == undefined){
		aid = id;
	}
	var arr = _._getParentChild(id);
	for(var i=1,c=arr.length; i<c; i++){
		var rData = _._getRowData(arr[i]);
		if(rData != undefined && !rData.isDisplay){
			var row = _._getRowObj(arr[i]);
			if(row != null){
				row.style.display = '';
				_._setRowDisplay(arr[i], true);
			}
		}
	...
	_._getRowData(aid).isExpand = true;
	_._setSwitch(aid);
	if(_.config.showIcon){
		_._setIcon(aid);
	}
};
	...
	for(var i=1,c=arr.length; i<c; i++){
		...
		if(rData != undefined && rData.isDisplay){
			...
			if(row != null){
				...
				_._setRowDisplay(arr[i], false);
			}
		}
		if(rData.hasChild){
			_.collapse(arr[i], aid);
		}
	}
	_._getRowData(aid).isExpand = false;
	_._setSwitch(aid);	
	if(_.config.showIcon){
		_._setIcon(aid);
	}
...
//以下功能用于缓存展开的节点，以防止刷新
oTreeTable.prototype.setCookie = function(name, value, expireMinutes){
	var _ = this;
...
oTreeTable.prototype.getCookie = function(name){
    var arr = document.cookie.match(new RegExp("(^| )" + name + "=([^;]*)(;|$$)"));
	return arr != null ? unescape(arr[2]) : '';
};

oTreeTable.prototype.delCookie = function(name) {
    var expDate = new Date();
...
oTreeTable.prototype._buildCookieName = function(){
	var _ = this;
	var page = location.href.split('?')[0];
	var pos = page.lastIndexOf('/');
	page = page.substr(pos + 1).split('.')[0];
	return _.id + 'TreeTableCache_' + page;
};

oTreeTable.prototype.expandCache = function(){
	var _ = this;
	var name = _._buildCookieName();
	if(_.config.isCache){
		var arr = _.getCookie(name).split('|');
		for(var i=0,c=arr.length; i<c; i++){
			_.collapse(arr[i]);
		}
	} else {
...
oTreeTable.prototype.cacheExpand = function(){
	var _ = this;
	if(_.config.isCache){
		var arr = [];
		for(var i in _.arrRowData){
			//缓存被折叠的节点
			if(!_.arrRowData[i].isExpand && _.arrRowData[i].hasChild){
				arr.push(_.arrRowData[i].id);
			}
		}
		_.setCookie(_._buildCookieName(), arr.join('|'), _.config.expireMinutes);
	}
};